...
interface MenuItemsReorderModalProps {
  opened: boolean;
  onClose: () => void;
  categoryName: string;
  ...
  loading: boolean;
  saving: boolean;
  onSave: (orderedItems: MenuItemSummary[]) => Promise<void>;
}
...
export const MenuItemsReorderModal: FC<MenuItemsReorderModalProps> = ({
  opened,
  onClose,
  categoryName,
  items,
  ...
  const [orderedItems, setOrderedItems] = useState<MenuItemSummary[]>([]);
  ...
  useEffect(() => {
    ...
    initialOrderRef.current = items.map((item) => item.itemId);
